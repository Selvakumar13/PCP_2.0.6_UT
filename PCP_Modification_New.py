import asyncio
import websockets
import json
import base64
import time
import os
import sys
import psutil
from utils import utilityFunctions
from win32 import win32api, win32print
from threading import Thread
from datetime import datetime
from windows_toasts import Toast, WindowsToaster
import tempfile
import logging
from logging.handlers import RotatingFileHandler
import subprocess
import sys
import socket
from PySide6.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QFrame, QMenu, QMessageBox, \
        QSystemTrayIcon, QLabel
from PySide6.QtGui import QIcon, QPixmap, QAction
from PySide6.QtCore import Qt, QByteArray
from utils.Utility import Printer_details as utility_1
from utils.Utility import App_status as utility_2

script_path = r'F:\Everything\PCP_Everything\PCP_Combined\utils\UT.py'

def is_port_in_use(port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        for i in port:
            return s.connect_ex(('localhost', i)) == 0

# Check if port 5000 is already in use
if is_port_in_use([5000,9876]):
        # Display a pop-up message
        app = QApplication(sys.argv)
        error_msg_box = QMessageBox()
        error_msg_box.setIcon(QMessageBox.Critical)
        error_msg_box.setWindowTitle("Error")
        error_msg_box.setText("PCP_2.0.6 is already running")
        base64_image = b""
        pixmap = QPixmap()
        pixmap.loadFromData(QByteArray.fromBase64(base64_image))
        error_msg_box.setWindowIcon(QIcon(pixmap))
        error_msg_box.setStandardButtons(QMessageBox.Ok)
        error_msg_box.exec()

        # You can add more actions or modify the message as needed

        # Exit the script
        sys.exit(1)


else:
    print("Port 5000 is available. Starting the PCP application.")

async def UT():

    class DesktopUtilityApp(QMainWindow):
        def __init__(self):
            super().__init__()

            # Create a socket to ensure only one instance can run
            self.socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            try:
                # Try to bind to a specific port (replace with your desired port)
                self.socket.bind(("127.0.0.1", 9876))
            except socket.error:
                # If the port is already in use, another instance is running
                self.show_error_message()
                sys.exit(1)

            self.setWindowTitle("PCP Utility")
            self.setGeometry(100, 100, 450, 300)

            # Your base64 image string goes here
            base64_image = b""
            pixmap = QPixmap()
            pixmap.loadFromData(QByteArray.fromBase64(base64_image))

            self.setWindowIcon(QIcon(pixmap))

            self.central_widget = QWidget()
            self.setCentralWidget(self.central_widget)

            layout = QVBoxLayout()

            self.printer_app = utility_1.PrinterValidationApp()
            layout.addWidget(self.printer_app)

            line = QFrame()
            line.setFrameShape(QFrame.HLine)
            line.setFrameShadow(QFrame.Sunken)
            layout.addWidget(line)

            self.software_app = utility_2.SoftwareCheckerApp()
            layout.addWidget(self.software_app)

            self.central_widget.setLayout(layout)

            # Set the style sheet (moved to the end of __init__)
            self.setStyleSheet("""
                                   QMainWindow {
                                       background-color:#f5f5f5; /* White background */
                                   }
                                   QLabel {
                                       color: #333333; /* Black text */
                                       font-size: 12px; /* Slightly smaller font */
                                   }
                                   QLineEdit {
                                       background-color: white;
                                       border: 1px solid #ccc; /* Light Gray border */
                                       padding: 4px;
                                       width: 150px; /* Adjust as needed */
                                   }
                                   QPushButton {
                                       background-color: #0d6efd; /* Dark Blue button */
                                       color: white;
                                       border: none;
                                       padding: 6px 12px;
                                       border-radius: 3px;
                                       width: 150px; 
                                   }
                                   QPushButton:hover {
                                       background-color: #0951ba; /* Slightly darker grey on hover */
                                   }
                               """)

            # Create a menu bar
            menubar = self.menuBar()

            # Add an About action to the Help menu
            about_action = QAction("About", self)
            about_action.triggered.connect(self.show_about_dialog)
            menubar.addAction(about_action)

            # Create a File menu
            exit_action = QAction("Exit", self)
            exit_action.triggered.connect(self.close_application)
            menubar.addAction(exit_action)
            # Create a system tray icon
            self.tray_icon = QSystemTrayIcon(self)
            self.tray_icon.setIcon(QIcon(pixmap))
            self.tray_icon.setToolTip("PCP Utility")
            self.tray_icon.setContextMenu(self.create_tray_menu())
            self.tray_icon.activated.connect(
                lambda reason: self.open_pcp_app() if reason == QSystemTrayIcon.ActivationReason.DoubleClick else None)

            self.tray_icon.show()

        def create_tray_menu(self):
            tray_menu = QMenu()

            open_action = QAction("Open PCP Utility", self)
            open_action.triggered.connect(self.open_pcp_app)
            tray_menu.addAction(open_action)

            exit_action = QAction("Exit", self)
            exit_action.triggered.connect(self.close_application)
            tray_menu.addAction(exit_action)

            return tray_menu

        def show_about_dialog(self):
            about_text = "PCP Utility\n\nVersion: 1.00\n\n"
            QMessageBox.about(self, "About PCP Utility", about_text)

        def show_error_message(self):
            error_msg_box = QMessageBox()
            error_msg_box.setIcon(QMessageBox.Critical)
            error_msg_box.setWindowTitle("Error")
            error_msg_box.setText("Another instance is already running!")
            error_msg_box.setInformativeText("Please close the existing instance before running a new one.")
            error_msg_box.setStandardButtons(QMessageBox.Ok)

            # Set the logo as the window icon for the QMessageBox
            base64_image = b""
            pixmap = QPixmap()
            pixmap.loadFromData(QByteArray.fromBase64(base64_image))
            error_msg_box.setWindowIcon(QIcon(pixmap))

            error_msg_box.exec()

        def open_pcp_app(self):
            self.show()

        def close_application(self):
            # Close the socket when the application is closed
            self.socket.close()

            # Stop the specified executable
            self.end_process()
            self.close()
            sys.exit()

        def end_process(self):
            # Get the list of running processes
            processes = [proc.info for proc in psutil.process_iter(['pid', 'name'])]

            # Display the list of processes to the user
            print("List of running processes:")
            for process in processes:
                print(f"{process['pid']}: {process['name']}")
                # Prompt the user for the process name or PID to terminate
                process_to_end = 'PCP_2.0.6_UT.exe'
                try:
                    # Attempt to terminate the specified process
                    if process_to_end == process['name']:
                        pid_to_end = process['pid']
                        psutil.Process(pid_to_end).terminate()
                        print(f"Process with PID {pid_to_end} terminated successfully.")
                    else:
                        for process in processes:
                            if process['name'] == process_to_end:
                                psutil.Process(process['pid']).terminate()
                                print(f"Process {process['name']} terminated successfully.")
                                break
                        else:
                            print(f"No process found with the name: {process_to_end}")
                except psutil.NoSuchProcess as e:
                    print(f"Error: {e}")
                except psutil.AccessDenied as e:
                    print(f"Error: {e}")

    if __name__ == "__main__":
        app = QApplication(sys.argv)
        window = DesktopUtilityApp()
        window.show()
        app.setQuitOnLastWindowClosed(False)
        sys.exit(app.exec())


async def run_script():
    try:
        # Call UT() function instead of running the script_path
        await UT()
    except Exception as e:
        print(f"Error running the UT function: {e}")



def run_script_thread():
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(run_script())



isToastAvailable = True
pcp_name = "PCP 2.0.6"
error_msg = ""
printer_list = []
logger = logging.getLogger("Error Log")
logger.setLevel(logging.INFO)
isToastAvailable, newToast, wintoaster = utilityFunctions.checkToasterAvailable(pcp_name)
handler = RotatingFileHandler(os.path.join(tempfile.gettempdir(), "OWMError.log"), maxBytes=1000000, backupCount=0,
                              encoding=None, delay=0)
logger.addHandler(handler)
jobExists = False
printers = win32print.EnumPrinters(win32print.PRINTER_ENUM_LOCAL, None, 1)
for printer in printers:
    printer_list.append(printer[1])


async def execute_print(websocket, json_data):
    # Get the current printer name and log it into the system
    logger.info(f"{datetime.now().strftime('%d-%m-%Y_%X')} Printer Name {json_data['printerName']}")
    try:

        win32print.SetDefaultPrinter(json_data['printerName'])
        _printer = win32print.GetDefaultPrinter()
        h_printer = win32print.OpenPrinter(_printer)
        status_attributes = win32print.GetPrinter(h_printer)[18]
        date_time = datetime.now().strftime('%d-%m-%Y_%X')
        handle = win32print.OpenPrinter(json_data['printerName'])
        # get the printer attribute
        attributes = win32print.GetPrinter(handle)[13]
        # Check if the printer is offline
        if (attributes & 0x00000400) >> 10 == 1:
            if isToastAvailable:
                newToast.text_fields = ["It Seems that your printer is offline"]
                wintoaster.show_toast(newToast)
            else:
                logger.info(
                    f"{datetime.now().strftime('%d-%m-%Y_%X')} It seems that the provided printer is offline")

            await websocket.send(json.dumps({"statusCode": 501,
                                             "status": "Failure",
                                             "message": "Printer is Offline",
                                             "payload": {"_id": json_data['_id'], "name": json_data['name'],
                                                         "printerName": json_data["printerName"]},
                                             "isPrinted": False}))
        else:
            # This block handles the case where the printer prints.
            if isToastAvailable:
                newToast.text_fields = ['Document is getting printed']
                wintoaster.show_toast(newToast)
            else:
                logger.info(f"{datetime.now().strftime('%d-%m-%Y_%X')} Document is getting printed")
                # Clean the bas64 data received from the client.
            pdf_base64 = json_data['pdf_base64'].split(',')[1]
            # Get the file name
            file = json_data["name"]
            # create the hidden folder in the temp folder
            tempdir = tempfile.gettempdir()
            result = utilityFunctions.create_folder_in_given_path(".OWM", tempdir)
            logger.info({f"{date_time}"}, result)
            filename = os.path.join(tempdir, ".OWM", f'{file}')
            with open(filename, 'wb') as theFile:
                theFile.write(base64.b64decode(pdf_base64))
            printThread = Thread(target=asyncio.run, args=(
                utilityFunctions.printToPrinter(tempdir, filename, json_data['printerName'], json_data["name"],
                                                 websocket,
                                                 json_data),))
            logger.info(f"{date_time}", "Document sent for printing")
            printThread.start()
            logger.info(f"{date_time}", "Document Out of printer")
    except Exception as e:
        logger.error(f"{datetime.now().strftime('%d-%m-%Y_%X')} {e}")
        print(e)
        if (e.args[0] == "<built-in function SetDefaultPrinter> returned a result with an exception set"):
            print(True)
            await websocket.send(json.dumps({"statusCode": 404,
                                             "status": "Failure",
                                             "message": f"Printer {json_data['printerName']} is not available.",
                                             "payload": {"_id": json_data['_id'], "name": json_data['name'],
                                                         "printerName": json_data["printerName"]},
                                             "isPrinted": False}))


async def server(websocket, path):
    try:
        async for data in websocket:
            try:
                json_data = json.loads(data)
                thread = Thread(target=asyncio.run, args=(execute_print(websocket, json_data),))
                thread.start()
            except Exception as error:
                print(error)
    except websockets.exceptions.ConnectionClosedError as e:
        print(e)
        logger.info({datetime.now().strftime('%d-%m-%Y_%X')}, e)


# Always make sure to wrap you code in a try, except block.
try:
    # Run the script in a separate thread
    script_thread = Thread(target=run_script_thread)
    script_thread.start()

    # Start the connection first
    start_server = websockets.serve(server, "localhost", 5000, max_size=9000000000)
    asyncio.get_event_loop().run_until_complete(start_server)
    # This ensures that the event loop runs forever.
    asyncio.get_event_loop().run_forever()
except Exception as e:
    # If an instance of the app is already running, This message appears Or if some other app is running on port 5000
    if isToastAvailable:
        newToast.text_fields = ["Port 5000 is occupied. Either the PCP is already running or some other application is running on port 5000"]
        wintoaster.show_toast(newToast)
    else:
        logger.info({datetime.now().strftime('%d-%m-%Y_%X')}, "An Instance of PCP is already Running")
